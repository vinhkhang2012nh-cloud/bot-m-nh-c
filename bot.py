...
def play_next(ctx):
    ...
    if guild_id in music_queues and len(music_queues[guild_id]) > 0:
        ...
        ctx.voice_client.play(
            player,
            after=lambda e: asyncio.run_coroutine_threadsafe(play_next_async(ctx), bot.loop)
        )
...
async def play(ctx, *, search: str):
    ...
    async with ctx.typing():
        ...
        if ctx.voice_client.is_playing():
            ...
            await ctx.send(f'📥 Đã thêm vào hàng đợi: **{song["title"]}** (Vị trí: {len(music_queues[guild_id])})')
        else:
...
if not token:
    print("⚠️ LỖI CHƯA CÓ TOKEN: Bạn hãy vào phần Environment trên Render tạo biến DISCORD_TOKEN và dán token vào nhé!")
else:
    bot.run(token)
